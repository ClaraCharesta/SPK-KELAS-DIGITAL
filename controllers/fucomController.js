...
if (await fucomModel.checkBobotSudahDihitung(periode.id_periode)) {
    return res.redirect('/admin/fucom?error=Bobot kriteria sudah dihitung. Data responden tidak dapat ditambah/diubah lagi.');
}
...
// VALIDASI BARU: nilai perbandingan wajib bilangan bulat 1-9, tanpa desimal/teks
for (const nilai of nilaiPerbandingan) {
    if (!/^[1-9]$/.test(String(nilai).trim())) {
        return res.redirect('/admin/fucom?error=Nilai perbandingan harus berupa bilangan bulat 1-9 (tanpa desimal atau teks).');
    }
}
...
if (await fucomModel.checkBobotSudahDihitung(periode.id_periode)) {
    return res.redirect('/admin/fucom?error=Bobot kriteria sudah dihitung. Data responden tidak dapat dihapus lagi.');
}
