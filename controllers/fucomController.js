...
async function cekPerankinganSudahDimulai(id_periode) {
    const [[hasil]] = await db.query(
        'SELECT COUNT(*) AS jumlah FROM hasil_waspas WHERE id_periode = ?',
        [id_periode]
    );
...
const fucomController = {
    ...
    async saveInput(req, res) {
        try {
            const periode = await periodeModel.getActivePeriode();
            if (await cekPerankinganSudahDimulai(periode.id_periode)) {
                return res.redirect('/admin/fucom?error=Pembobotan terkunci karena perankingan sudah pernah dijalankan. Hapus hasil perankingan di menu Perankingan terlebih dahulu jika ingin mengubah bobot.');
            }
    ...
    async hitungBobot(req, res) {
        try {
            ...
            if (await cekPerankinganSudahDimulai(periode.id_periode)) {
                return res.redirect('/admin/fucom?error=Pembobotan terkunci karena perankingan sudah pernah dijalankan.');
            }
            ...
            for (const nama of respondenList) {
                ...
                semuaBobotResponden.push(hasil.bobot);
                totalDfc += hasil.dfc;
            }

            const bobotAgregasi = agregasiBobot(semuaBobotResponden);
            const dfcFinal = totalDfc / respondenList.length;

            await fucomModel.saveBobotFinal(periode.id_periode, bobotAgregasi, dfcFinal);
            await userModel.logActivity(req.session.user.id_user, `Menghitung bobot kriteria FUCOM (agregasi ${respondenList.length} responden)`);
